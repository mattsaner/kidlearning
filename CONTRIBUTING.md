...
Drawings live in `js/drawings.js` as a list of regions in a 100 x 100 box. Each region is `{ c: '<color id>', d: '<SVG path>' }`, where `c` is a color id from `js/data.js` (the brush color and the spoken name come from it). Regions are in painter's order: later regions are drawn on top of earlier ones and may overlap them. Use the helpers (`rect`, `circle`, `ellipse`, `poly`) and keep regions large (toddler hands!). Then add the drawing to `DRAWINGS` with names in en/fr/de.
